...
export default function RiderDashboardPage() {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      ordersChannel = supabase.channel("rider-orders-" + userId);
      ordersChannel
        .on(
          "postgres_changes",
          {
  ...
  return (
    <div className="min-h-screen bg-gray-100" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {/* HEADER */}
      <header className="sticky top-0 z-50 flex h-16 items-center justify-between border-b border-gray-200 bg-[#ffffff] px-4 md:px-6">
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          {/* Rider info — hidden on very small screens */}
          {rider && (
            <div className="hidden items-center gap-2 sm:flex">
              <div className="flex h-8 w-8 items-center justify-center rounded-full border border-[#333] bg-gray-100">
          ...
          <div className="relative" ref={notifRef}>
            ...
            {/* NOTIF DROPDOWN */}
            {notifOpen && (
              <div className="absolute right-0 top-11 z-50 w-75 overflow-hidden rounded-2xl border border-[#6b6b6b] bg-gray-200 shadow-2xl sm:w-80">
                  ...
                  {notifications.length === 0 ? (
                    ...
                    notifications.map((n) => (
                      <div
                        ...
                        className="flex cursor-pointer items-start gap-2 border-b border-[#6b6b6b] px-4 py-3 hover:bg-[#1a1a1a]"
                        style={{ background: !n.read ? "#e9e9e9" : "transparent" }}
                      >
                        {!n.read && (
                        ...
                        <div className={!n.read ? "" : "ml-3"}>
                          ...
                          <p className="mt-0.5 text-[11px] text-gray-600">{n.body}</p>
                ...
                {notifications.length > 0 && (
                  <div className="border-t border-[#6b6b6b] px-4 py-2.5">
                    <button
                      onClick={async () => {
                        const { data: { user } } = await supabase.auth.getUser();
                        if (user) markAllRead(user.id);
                      }}
                      className="text-xs align-middle w-full font-semibold text-black hover:text-white"
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg border border-[#494949] bg-gray-100 px-3 py-2 text-xs font-semibold text-[#888]"
      ...
      <main className="mx-auto max-w-5xl px-4 py-6 md:px-6 md:py-8">
        ...
        {/* STAT CARDS — 3 cols always, shrink gracefully */}
        <div className="mb-6 grid grid-cols-3 gap-3 md:gap-4">
          {[
            ...
            <div
              key={stat.label}
              className="rounded-2xl border border-gray-300 bg-white p-4 md:p-5"
            >
              <div
              ...
              <p className="mt-1 text-2xl font-bold tracking-tight text-black md:text-3xl">
                {stat.value}
              </p>
            </div>
          ))}
        ...
        {loading ? (
          ...
          <div className="flex h-48 flex-col items-center justify-center rounded-2xl border border-gray-300 bg-white text-[#444]">
          ...
          <div className="flex flex-col gap-3">
            {displayedOrders.map((order) => (
              <div
                key={order.id}
                className="order-card rounded-2xl border border-gray-300 bg-white p-4 md:p-6"
              >
                {/* Order top row */}
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <a
                        ...
                        className="text-xs font-semibold text-blue-400 no-underline"
                      >
